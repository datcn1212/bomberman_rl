...
REPORT_SARSA = ROOT / "report_SARSA.md"
...
CLAIMS = [
    ...
    ("Phase S1 SARSA target",           "sarsa_four",      3, "mean_score", 1.717),
    ("Phase S1 SARSA coins",            "sarsa_four",      3, "mean_coins", 1.141),
]

# report_SARSA.md quotes the Q-learning control as the mean of the three repeat
# runs rather than as one experiment, so it is checked as a derived figure.
DERIVED = [
    ("Phase S1 Q control (3 pooled)", ["p19b_6000ep", "final_tabular_q", "repeat_c"],
     "mean_score", 2.275),
]
...
def main():
    ...
    for label, exp_ids, key, quoted in DERIVED:
        parts = [per_seed(rows, e, 3, key) for e in exp_ids]
        if any(p is None for p in parts):
            unchecked += 1
            print("  [MISSING] %-30s one of %s has no rows" % (label, exp_ids))
            continue
        actual = statistics.mean([statistics.mean(p) for p in parts])
        ok = abs(actual - quoted) < TOLERANCE
        failures += not ok
        print("  [%s] %-30s report %7.3f   data %7.3f   (%d runs)"
              % ("OK   " if ok else "STALE", label, quoted, actual, len(parts)))
    ...
    if failures:
        ...
        print("ALL %d QUOTED FIGURES MATCH" % (len(CLAIMS) + len(DERIVED) + 1))
