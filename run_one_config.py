"""Train + eval MOT config cho agent q_linear (Task 1, coin-heaven).

Day la cong cu phat trien dung de chay thi nghiem hyperparameter, KHONG
phai mot phan code nop bai (khong nam trong agent_code/q_linear/).

Doc hyperparameter tu bien moi truong, chay qua main.py co san (khong sua
framework), luu ket qua vao results/eval_<name>.json.
"""
import json
import os
import subprocess
...
ALPHA = os.environ.get("Q_LINEAR_ALPHA", "0.05")
GAMMA = os.environ.get("Q_LINEAR_GAMMA", "0.95")
USE_SHAPING = os.environ.get("Q_LINEAR_USE_SHAPING", "1")
...
def main():
    ...
    run(
        [sys.executable, "main.py", "play", "--agents", "q_linear", "--train", "1",
         "--no-gui", "--scenario", "coin-heaven", "--n-rounds", str(TRAIN_EPISODES),
         "--log-dir", str(LOG_DIR)],
        {
            ...
            "Q_LINEAR_USE_SHAPING": USE_SHAPING,
        },
    )
    t1 = time.time()
    ...
    n = len(rounds)
    coins_mean = sum(coins) / n
    steps_mean = sum(steps) / n

    print(f"[{NAME}] train={t1 - t0:.1f}s eval={t2 - t1:.1f}s "
          f"coins_mean={coins_mean:.2f} steps_mean={steps_mean:.1f} n_rounds={n}")
