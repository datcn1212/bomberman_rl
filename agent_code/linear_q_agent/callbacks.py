...
def setup(self):
    ...
    if self.train and not os.path.isfile("q_table.pkl"):
        ...
        self.model = LinearQModel(learning_rate=self.lr, gamma=self.gamma, actions=ACTIONS, n_features=8)
    elif self.train:
        ...
        self.model = LinearQModel(learning_rate=0.0, gamma=self.gamma, actions=ACTIONS, n_features=8)
        self.epsilon = 0.0
        with open(weights_file, "rb") as file:
            self.model.load(file)
